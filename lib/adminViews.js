    ...
    <div class="card">
      ...
      <form method="post" action="/${adminPath}/orders/${escapeHtml(order.id)}/quote" class="grid">
        ...
        <div class="grid two">
          <label>
            Other item label
            <input type="text" name="otherLabel" value="${escapeHtml(quote.otherLabel)}" />
          </label>
          <label>
            Other item amount
            <input type="number" name="otherAmount" step="0.01" min="0" value="${escapeHtml(quote.otherAmount)}" />
          </label>
        </div>
        <label>

...
function buildCustomerEmail(order, t) {
  ...
  if (termsLine && t.email.termsLabel) {
    htmlBlocks.push(
...
function buildQuoteEmail(order, quote, t) {
  ...
  if (termsLine && t.quote.termsLabel) {
    htmlBlocks.push(
      `<p><a href="${escapeHtml(termsUrl)}" style="color:#5ff0c7;text-decoration:underline;">${escapeHtml(
        t.quote.termsLabel
      )}</a></p>`
...
async function sendQuoteEmail(order, quote) {
  const t = getLocale(order.language);
  const quoteEmail = buildQuoteEmail(order, quote, t);
  await sendEmail({
    to: [order.email],

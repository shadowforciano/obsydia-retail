...
function getTermsUrl(baseUrl) {
  const resolved = String(baseUrl || process.env.PUBLIC_BASE_URL || '').trim();
  if (!resolved) {
    return null;
  }
  return `${resolved.replace(/\/+$/, '')}/terms`;
}
...
function buildCustomerEmail(order, t, options = {}) {
  ...
  const termsUrl = getTermsUrl(options.baseUrl);
  const termsLine = t.email.termsLine
    ? t.email.termsLine.replace(/\{\{\s*termsUrl\s*\}\}/g, termsUrl || '')
    : '';
  ...
  if (termsLine && termsUrl) {
    textLines.push('', termsLine);
  }
  ...
  if (termsLine && termsUrl && t.email.termsLabel) {
    htmlBlocks.push(
      `<p><a href="${escapeHtml(termsUrl)}" style="color:#5ff0c7;text-decoration:underline;">${escapeHtml(
        t.email.termsLabel
      )}</a></p>`
    );
  }
...
function buildAdminEmail(order, t) {
  ...
  const summaryItems = [
    ...
    { label: labels.language, value: languageName },
    { label: labels.orderId, value: order.id },
    { label: labels.timestamp, value: formatAdminTimestamp(order.createdAt) },
  ];
...
function buildQuoteEmail(order, quote, t, options = {}) {
  const currency = quote.currency || 'USD';
  const formatter = new Intl.NumberFormat('en-US', { style: 'currency', currency });
  const items = Array.isArray(quote.items) ? quote.items : [];
  const notes = quote.notes || '';
  const paymentInstructions = applyPaymentNumber(t.quote.paymentInstructions).replace(
    /\{\{\s*orderId\s*\}\}/g,
    order.id
  );
  const termsUrl = getTermsUrl(options.baseUrl);
  const termsLine = t.quote.termsLine
    ? t.quote.termsLine.replace(/\{\{\s*termsUrl\s*\}\}/g, termsUrl || '')
    : '';

  const labeledItems = items
    .filter((item) => Number(item.amount) > 0)
    .map((item) => {
      const fallbackLabel = t.quote.items?.[item.key] || item.key;
      const label = item.key === 'other' && item.label ? item.label : fallbackLabel;
      return {
        label,
  ...
  const total = formatter.format(Number(quote.total || 0));

  const textLines = [
    t.quote.intro,
    '',
    `${t.quote.orderIdLabel}: ${order.id}`,
    t.quote.currencyNote,
    '',
    t.quote.itemsTitle,
    ...labeledItems.map((item) => `${item.label}: ${item.amount}`),
    `${t.quote.totalLabel}: ${total}`,
    '',
    t.quote.paymentTitle,
    paymentInstructions,
    notes ? `\n${t.quote.notesLabel}: ${notes}` : '',
    '',
    t.quote.outro,
  ];
  ...
  const htmlBlocks = [
    `<p>${escapeHtml(t.quote.intro)}</p>`,
    `<p><strong>${escapeHtml(t.quote.orderIdLabel)}:</strong> ${escapeHtml(order.id)}</p>`,
    `<p>${escapeHtml(t.quote.currencyNote)}</p>`,
    `<h3>${escapeHtml(t.quote.itemsTitle)}</h3>`,
    '<ul>',
    ...labeledItems.map(
      (item) =>
        `<li><strong>${escapeHtml(item.label)}:</strong> ${escapeHtml(item.amount)}</li>`
    ),
    '</ul>',
    `<p><strong>${escapeHtml(t.quote.totalLabel)}:</strong> ${escapeHtml(total)}</p>`,
    `<h3>${escapeHtml(t.quote.paymentTitle)}</h3>`,
    `<p>${escapeHtml(paymentInstructions)}</p>`,
    notes ? `<p><strong>${escapeHtml(t.quote.notesLabel)}:</strong> ${escapeHtml(notes)}</p>` : '',
    `<p>${escapeHtml(t.quote.outro)}</p>`,
  ];

  if (termsLine && termsUrl && t.quote.termsLabel) {
    htmlBlocks.push(
      `<p><a href="${escapeHtml(termsUrl)}" style="color:#5ff0c7;text-decoration:underline;">${escapeHtml(
        t.quote.termsLabel
      )}</a></p>`
  ...
  return {
    subject: t.quote.subject.replace('{{orderId}}', order.id),
    text,
    html: buildEmailShell(t.quote.subject.replace('{{orderId}}', order.id), htmlContent),
  };
}
...
async function sendOrderEmails(order, options = {}) {
  ...
  const customerEmail = buildCustomerEmail(order, t, options);
...
async function sendQuoteEmail(order, quote, options = {}) {
  const t = getLocale(order.language);
  const quoteEmail = buildQuoteEmail(order, quote, t, options);
  await sendEmail({
    to: [order.email],
